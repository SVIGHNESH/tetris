...
A C++20 reimplementation of Stephen Brennan's Tetris, built as a learning
exercise in modern C++ and testable architecture. The engine (`tetris_core`)
knows nothing about ncurses and does not link it, so the whole game is testable
without a terminal. `PLAN.md` holds the design reasoning and the deliberate
...
New here? `blueprint/README.md` explains the whole workflow. The root `README.md`
...
C++20 + CMake. No package manager; ncurses comes from the system and Catch2 is
fetched at configure time (the first configure needs network access).

- Configure: `cmake -S . -B build`
- Build: `cmake --build build`
- Test: `ctest --test-dir build`
- Run: `./build/tetris` (`--rows N`, `--cols N`, `--seed N`)

Useful configure flags:

- `-DTETRIS_BUILD_UI=OFF` - engine and tests only, no ncurses needed
- `-DTETRIS_BUILD_TESTS=OFF` - skip Catch2 entirely

There is no separate lint step. The build is the lint: every target compiles with
`-Wall -Wextra -Wpedantic -Werror -Wconversion -Wshadow`.

A `test` command is declared, so **the test gate is on**. A step that adds engine
logic must ship a passing test in the same diff, and `ctest` must be green before
a checkpoint commit or `/complete`. UI steps ride on running the binary instead.

No combined `Verify` command and no GitHub checks exist yet. Run `/ci` or `$ci` to
set them up.
